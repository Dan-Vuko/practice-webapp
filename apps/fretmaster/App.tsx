...
import { TUNINGS, KEYS, FRET_COUNT, STRUCTURES, COLOR_PALETTE, RING_COLOR_PALETTE, SARGAM_NAMES, INTERVAL_NAMES, INTERVAL_COLORS, DEFAULT_THEME, ROMAN_DEGREES } from './constants';
import type { Tuning, HighlightedNote, Color, RingColor, StructureLabelType, StructureKey, SavedPattern, Structure, StringGroup, Instrument, FretboardTheme, FretboardInstance } from './types';
...
const App: React.FC = () => {
  ...
  const toggleIntervalVisibility = useCallback((interval: number) => {
    if (activeFretboard.isAdvancedMode && activeFretboard.activeGroupId) {
      const newGroups = activeFretboard.stringGroups.map(g => {
        if (g.id === activeFretboard.activeGroupId) {
      ...
      const newSet = new Set(activeFretboard.visibleIntervals);
      if (newSet.has(interval)) newSet.delete(interval);
      else newSet.add(interval);
      updateActiveFretboard({ visibleIntervals: newSet });
    }
  }, [activeFretboard, updateActiveFretboard]);
  ...
  const setAllIntervalsVisibility = useCallback((visible: boolean) => {
    ...
    if (activeFretboard.isAdvancedMode && activeFretboard.activeGroupId) {
      const newGroups = activeFretboard.stringGroups.map(g => g.id === activeFretboard.activeGroupId ? { ...g, visibleIntervals: newSet } : g);
      updateActiveFretboard({ stringGroups: newGroups });
    } else updateActiveFretboard({ visibleIntervals: newSet });
  }, [activeFretboard, updateActiveFretboard]);
  ...
  const strumAll = useCallback(() => {
    ...
    if (activeFretboard.isAdvancedMode && activeGroup) {
      ...
      tuning.strings.forEach((openNote, sIdx) => {
